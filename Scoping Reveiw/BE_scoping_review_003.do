...
*DO FILE META DATA
    //  algorithm name          BE_scoping_review_003
    ...
    //  algorithm task          Creating graphics of basic descriptives of included studies
    //  status                  Ongoing
...
mrgraph hbar 
			...
			aims_countries___9 
				if data_extractor == 1 & status == 1, 
						stat(column) ytitle("Pecentage (%)") 
						bar(1, fcolor(green) fintensity(inten60)) 
						blabel(bar, format(%9.1f)) 
						ylabel(0(10)80, nogrid labsize(small))
...
mrgraph hbar
			be_gis___1 be_gis___2 be_gis___3 be_gis___4 
					if data_extractor == 1 & status == 1, 
						...
						blabel(bar, format(%9.1f)) 
						ylabel(0(10)100, nogrid labsize(vsmall))
...
graph hbar
		if data_extractor == 2 & status == 1,
			...
			blabel(bar, format(%9.1f)) 
			ylabel(0(10)100, nogrid labsize(small))
			ytitle("Pecentage (%)") 
